...
-- Q-3. Write an SQL query to fetch unique values of DEPARTMENT from Worker table. (also by using group by)
...
-- Q-51 Remove all reverse elemnt
create table  pairs(
A int,
B int
);
...
select lt.* from pairs lt left join pairs as rt on lt.A = rt.B and lt.B = rt.A where rt.A is null or lt.A < rt.A;

-- METHOD 2 BY USING CORELATED SUB QUERY
